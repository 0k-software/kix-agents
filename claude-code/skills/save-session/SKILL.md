...
left). Then emit the handoff payload as **plain markdown** between two visible
sentinel lines so the user can select once (Cmd/Ctrl-shift to the closing
sentinel) and copy. **No outer code fence** — claude.ai's renderer doesn't
reliably preserve nested 3-backtick fences inside a 4-backtick or 5-backtick
outer fence (tested 2026-05-14; the inner files render mangled). The two
3-backtick fences for the inner files render as code blocks; the surrounding
prose renders as prose; the user copies the whole region. Template:
...
Above the block in the chat reply, a single one-liner: "Copy everything
between `--- BEGIN COPY ---` and `--- END COPY ---` and paste it into a
Claude Code session at `<owner/repo>`." Nothing else above it — the user
shouldn't have to scroll past anything to copy.
